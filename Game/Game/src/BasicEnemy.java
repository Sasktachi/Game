	...
	public void ai() {    
		...
		for(int i = 0; i < handler.object.size(); i++){
    		...
    		if(tempObject.getId() == ID.Player){
    			//If player is within 100 pixels
    			if(Player.abs((x - tempObject.getX())) <= 100){
    				//If player is on left, enemy moves left
    				if(tempObject.getX() < x) velX = -10;
    				//If player is on right, enemy moves right
    				if(tempObject.getX() > x) velX = 10;
    			}
    			//If player is more than 200 pixels away enemy stops moving
    			if(Player.abs((x - tempObject.getX())) >= 200) velX = 0;
